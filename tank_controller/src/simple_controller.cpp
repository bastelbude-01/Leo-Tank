#include "tank_controller/simple_controller.hpp"
    ...
    : Node(name)
{
    ...
    declare_parameter("wheel_seperation", 0.163);

    wheel_radius_ = get_parameter("wheel_radius").as_double();
    wheel_seperation_ = get_parameter("wheel_seperation").as_double();

    RCLCPP_INFO_STREAM(get_logger(), "WheelRadius beträgt : " << wheel_radius_);
    RCLCPP_INFO_STREAM(get_logger(), "WheelAbsatand beträgt : " << wheel_seperation_);

    wheel_cmd_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>("/simple_velocity_controller/commands", 10);

    vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>("/tank_controller/cmd_vel", 10, std::bind(&SimpleController::velCallback, this, _1));

    speed_conversion_ << wheel_radius_/2, wheel_radius_/2,
        wheel_radius_/wheel_seperation_, -wheel_radius_/wheel_seperation_;

    RCLCPP_INFO_STREAM(get_logger(), "Die Umwandlungs Matrix lautet wie folgt \n " << speed_conversion_<<"\n");
}

...
#define L298N_in3 A1  // Dir Motor B
#define L298N_in4 A2  // Dir Motor B

// #define L298N_enC 11  // PWM
// #define L298N_in5 A3  // Dir Motor A
// #define L298N_in6 A4  // Dir Motor A
...
// #define turret_encoder_phaseA 7   // Interrupt
// #define turret_encoder_phaseB 8
...
// unsigned int pipe_encoder_counter = 0;
// String turret_sign = "p";  // 'p' = positive, 'n' = negative
// String pipe_sign = "p";  // 'p' = positive, 'n' = negative
...
// bool is_turret_right = true;
// bool is_pipe_lift_up = true;
char value[] = "00.00";
uint8_t value_idx = 0;
...
// double turret_cmd_vel = 0.0;     // rad/s
// double pipe_cmd_vel = 0.0;      // rad/s

// Input - Measurement
double right_wheel_meas_vel = 0.0;    // rad/s
double left_wheel_meas_vel = 0.0;     // rad/s
// double  turret_meas_vel = 0.0;    // rad/s
// double  pipe_meas_vel = 0.0;     // rad/s
// Output - Command
double right_wheel_cmd = 0.0;             // 0-255
double left_wheel_cmd = 0.0;              // 0-255
// double turret_cmd = 0.0;             // 0-255
// double pipe_cmd = 0.0;              // 0-255
...
// PID turretMotor(&turret_meas_vel, &turret_cmd, &turret_cmd_vel, Kp_t, Ki_t, Kd_t, DIRECT);
// PID pipeMotor(&pipe_meas_vel, &pipe_cmd, &pipe_cmd_vel, Kp_p, Ki_p, Kd_p, DIRECT);

void setup() {
  // Init L298N H-Bridge Connection PINs
  pinMode(L298N_enA, OUTPUT);
  pinMode(L298N_enB, OUTPUT);
  // pinMode(L298N_enC, OUTPUT);
  ...
  // pipeMotor.SetMode(AUTOMATIC);
  Serial.begin(115200);

  // Init encoders
  pinMode(right_encoder_phaseB, INPUT);
  pinMode(left_encoder_phaseB, INPUT);
  // pinMode(turret_encoder_phaseB, INPUT);
  ...
  // attachInterrupt(digitalPinToInterrupt(turret_encoder_phaseA), turretEncoderCallback, RISING);
}
...
//      is_pipe_cmd = false;
    ...
    */
    ...
    else if(chr == 'p')
    {
      ...
      /*
      else if(is_turret_cmd && !is_turret_right)
      {
        // change the direction of the rotation
        digitalWrite(L298N_in5, HIGH - digitalRead(L298N_in5));
        digitalWrite(L298N_in6, HIGH - digitalRead(L298N_in6));
        is_turret_forward = true;
      }
    ...
    else if(chr == 'n')
    {
      ...
      /*
      else if(is_turret_cmd && is_turret_forward)
      {
        // change the direction of the rotation
        digitalWrite(L298N_in5, HIGH - digitalRead(L298N_in5));
        digitalWrite(L298N_in6, HIGH - digitalRead(L298N_in6));
        is_turret_forward = false;
      }
      */
    }
    // Separator
    else if(chr == ',')
    {
      ...
      /*
      else if(is_turret_cmd)
      {
        turret_cmd_vel = atof(value);
      }
      else if(is_pipe_cmd)
      {
        pipe_cmd_vel = atof(value);
        is_cmd_complete = true;
      }
      */
      // Reset for next command  Liste erweitern
      value_idx = 0;
      value[0] = '0';
      value[1] = '0';
      value[2] = '.';
  ...
  if(current_millis - last_millis >= interval)
  {
    ...
    // turret_meas_vel = (10 * turret_encoder_counter * (60.0 / 516000 )) * 0.10472;
    // pipe_meas_vel = (10 * pipe_encoder_counter * (60.0 / 516000 )) * 0.10472;
...
{
  if(digitalRead(turret_encoder_phaseB) == HIGH)
  {
    turret_sign = "n";
  }
  else
